...
import type { ServiceCatalogItem, TicketPriority } from '@/lib/types'
import { ArrowLeft, CheckCircle, ShieldCheck } from 'lucide-react'
import Link from 'next/link'

export function CatalogTicketForm({ item }: { item: ServiceCatalogItem | null }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.from('tickets').insert({
      ...
      title: title.trim(),
      description: description.trim() || item?.description || title,
      type: item ? 'request' : 'incident',
      area: item?.area ?? null,
    ...
    setLoading(false)
    if (error) {
      toast('Não foi possível abrir o chamado', 'error')
      return
    }
  ...
  return (
    <div className="mx-auto max-w-2xl p-6 space-y-5">
      ...
      <div>
        <h1 className="text-lg font-semibold text-zinc-100">{item?.title ?? 'Abrir chamado'}</h1>
        <p className="mt-1 text-[13px] text-zinc-500">{item?.instructions ?? 'Conte para a equipe de TI como podemos ajudar.'}</p>
      </div>
      ...
      <Card><CardContent className="pt-5">
        <form onSubmit={submit} className="space-y-4">
          {!item && (
            <Input label="Assunto" value={title} onChange={(e) => setTitle(e.target.value)} required />
          )}
          ...
          <Textarea
            label={item ? 'Informações adicionais' : 'Descrição'}
            value={description}
